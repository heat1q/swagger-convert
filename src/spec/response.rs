...
mod tests {
    ...
    fn deserialize_responses() {
        let responses =
            include_json!("../../tests/data/swagger.json", "/responses").to_string();
        let _responses: Responses = serde_json::from_str(&responses).unwrap();
    }

    #[test]
    fn into_openapi_responses() {
        let responses_raw = include_json!("../../tests/data/swagger.json", "/responses").to_string();
        let responses_openapi_raw =
            include_json!("../../tests/data/openapi.json", "/components/responses");
